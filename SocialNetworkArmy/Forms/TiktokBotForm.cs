...
namespace SocialNetworkArmy.Forms
{
    public partial class TiktokBotForm : Form
    {
        ...
        private TikTokCookieManagerWebView downloadService;
        private TestService testService;
        private readonly Profile profile;
        private readonly AutomationService automationService;
        private readonly ProxyService proxyService;
        private TikTokCookieManagerWebView cookieManager;
...
");
                ...
                // ========================================
                // 🍪 NOUVEAUTÉ : CHARGEMENT DES COOKIES
                // ========================================

                // 1. Initialiser le gestionnaire de cookies
                cookieManager = new TikTokCookieManagerWebView();

                // 2. Vérifier si cookies.txt existe
                if (cookieManager.CookiesFileExists())
                {
                    logTextBox.AppendText("[🍪 Cookies] Fichier cookies.txt détecté !\r\n");
                    cookieManager.PrintCookieInfo((msg) => logTextBox.AppendText($"[🍪 Cookies] {msg}\r\n"));
                }
                else
                {
                    logTextBox.AppendText("[⚠ Cookies] Aucun cookies.txt trouvé\r\n");
                    logTextBox.AppendText("[💡 Cookies] Créez cookies.txt avec l'extension 'Get cookies.txt LOCALLY'\r\n");
                    logTextBox.AppendText("[💡 Cookies] Placez-le dans le dossier : " + AppDomain.CurrentDomain.BaseDirectory + "\r\n");
                }

                // 3. Naviguer vers TikTok
                webView.CoreWebView2.Navigate("https://www.tiktok.com/");
                await Task.Delay(2000); // Attendre que la page charge

                // 4. Charger les cookies APRÈS la navigation initiale
                if (cookieManager.CookiesFileExists())
                {
                    logTextBox.AppendText("[🍪 Cookies] Injection des cookies dans WebView2...\r\n");
                    bool success = await cookieManager.LoadCookiesIntoWebView(
                        webView.CoreWebView2,
                        (msg) => logTextBox.AppendText($"[🍪 Cookies] {msg}\r\n")
                    );

                    if (success)
                    {
                        logTextBox.AppendText("[✓ Cookies] Cookies chargés avec succès !\r\n");

                        // Vérifier si on est connecté
                        bool isLoggedIn = await cookieManager.IsUserLoggedInAsync(webView.CoreWebView2);
                        if (isLoggedIn)
                        {
                            logTextBox.AppendText("[✓ Cookies] Utilisateur TikTok authentifié !\r\n");
                            logTextBox.AppendText("[✓ Cookies] Le bot ne sera PAS kické par TikTok 🎉\r\n");
                        }
                        else
                        {
                            logTextBox.AppendText("[⚠ Cookies] Cookies chargés mais authentification non détectée\r\n");
                            logTextBox.AppendText("[💡 Cookies] Les cookies sont peut-être expirés - re-exportez-les\r\n");
                        }

                        // Recharger la page pour appliquer les cookies
                        logTextBox.AppendText("[🔄 Cookies] Rechargement de TikTok avec les cookies...\r\n");
                        webView.CoreWebView2.Reload();
                        await Task.Delay(2000);
                    }
                    else
                    {
                        logTextBox.AppendText("[❌ Cookies] Échec du chargement des cookies\r\n");
                    }
                }

                // ========================================
                // FIN DU CHARGEMENT DES COOKIES
                // ========================================
                ...
                // Message final selon l'état des cookies
                if (cookieManager.CookiesFileExists())
                {
                    logTextBox.AppendText("[✓ Bot] Prêt avec authentification TikTok (cookies actifs) 🚀\r\n");
                }
                else
                {
                    logTextBox.AppendText("[⚠ Bot] Prêt SANS authentification (mode limité) ⚠\r\n");
                    logTextBox.AppendText("[💡 Bot] Pour éviter les kicks, ajoutez cookies.txt\r\n");
                }
